            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                ...
                @auth
                    @if(Auth::user()->role === 'user')
                        ...
                        @endphp
                        <a href="{{ route('cart.index') }}" class="relative p-0 text-gray-500 hover:text-gray-700 focus:outline-none focus:text-gray-700 transition duration-150 ease-in-out">
                            <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                            @if($cartCount > 0)
                                <span class="absolute -top-2 -left-2 inline-flex items-center justify-center rounded-full border border-transparent w-4 h-4 px-2 py-1 text-xs font-medium bg-red-600 text-white">{{ $cartCount }}</span>
                            @endif
                        </a>
                    @endif
