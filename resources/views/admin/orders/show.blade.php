                ...
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex justify-between items-center mb-6">
                        <div class="flex items-center">
                            <h1 class="text-2xl font-bold text-gray-800">Order #{{ $order->orders_id }}</h1>
                    ...
                    @if(count($availableStatuses) > 0)
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                            <h3 class="text-lg font-medium text-blue-900 mb-3">Update Order Status</h3>
                            ...
                            <div class="mt-3 text-sm text-blue-700">
                                <p><strong>Available transitions:</strong> {{ implode(', ', array_map('ucfirst', $availableStatuses)) }}</p>
                            </div>
                        </div>
                    @else
                        <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6">
                            ...
                            <p class="text-gray-600">
                                This order is in <strong>{{ $order->formatted_status }}</strong> status and cannot be changed further.
                            </p>
                        </div>
                    @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                        ...
                        <div class="bg-white border border-gray-200 rounded-lg p-4">
                            <h3 class="text-lg font-medium text-gray-900 mb-3">Customer</h3>
                            <div class="space-y-2">
                        ...
                        <!-- Order Info Card -->
                        <div class="bg-white border border-gray-200 rounded-lg p-4">
                            <h3 class="text-lg font-medium text-gray-900 mb-3">Order Info</h3>
                            <div class="space-y-2">
                        ...
                        <!-- Total Card -->
                        <div class="bg-white border border-gray-200 rounded-lg p-4">
                            <h3 class="text-lg font-medium text-gray-900 mb-3">Total</h3>
                            <div class="text-3xl font-bold text-green-600">{{ number_format($order->total_amount, 0) }}đ</div>
                            <div class="text-sm text-gray-600 mt-1">
                                {{ $order->delivery_date ? 'Delivery: ' . $order->delivery_date->format('M d, Y') : 'Delivery date not set' }}
                            </div>
                        </div>
                    </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($order->orderItems as $item)
                                        <tr>
                                            <td class="px-6 py-4">
                                                <div class="text-sm font-medium text-gray-900">{{ $item->product->product_name ?? 'Product not found' }}</div>
                                            </td>
