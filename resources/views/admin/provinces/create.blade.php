    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 color-gray-900">
                    <h1 class="text-2xl font-bold mb-4 color-black">{{ __('common.create_province') }}</h1>
                    ...
                    <form action="{{ route('admin.provinces.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="shipping_fee" id="shipping_fee" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" value="{{ old('shipping_fee') }}" required>
                        </div>
                        <div class="flex items-center justify-end">
                            <a href="{{ route('admin.provinces.index') }}" class="text-gray-600 hover:text-gray-900 mr-4">{{ __('common.cancel') }}</a>
                            <button type="submit" class="bg-gray-900 text-white font-bold py-2 px-4 rounded">{{ __('common.create') }}</button>
                        </div>
                    </form>
                </div>
            </div>
